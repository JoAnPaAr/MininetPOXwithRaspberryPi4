...
def myNetwork():
    ...
    #el controlador
    c0 = net.addController('c0', controller=RemoteController, ip='192.168.117.234', port=6633 )
    ...
    s22.cmd("ip link add s22-tfg1 type gretap local 192.168.117.31 remote 192.168.117.234 ttl 64")
    s22.cmd("ip link set s22-tfg1 up")
    
    #Anyadiendo las interfaces al switch de la topologia
    Intf("s22-tfg1",node=s22)
    net.start()
    CLI(net)
    
    info('***Stopping network')
    #Elimina las interfaces que se han creado para esta topologia
    s22.cmd("ip link del dev s22-tfg1")
    net.stop()
